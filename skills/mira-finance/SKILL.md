...
Phase F17A permits outbound customer delivery only through `mira_finance_prepare_delivery` followed by `mira_finance_confirm_delivery`. Email is the default. Select WhatsApp only when Ray explicitly asks for it. Always show the masked destination, document number, channel, amount, and expiring token before confirmation. Never invent or accept an ad-hoc destination, use the broad messaging tool, send a DOCX, bypass contact verification or WhatsApp consent, resend without a reason, or claim a failed/uncertain attempt was delivered. Do not read or process customer replies; that remains F17B.
