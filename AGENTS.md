...
Mira remains isolated and only the dedicated `RC Finance` WhatsApp group routes finance commands to her. F20 requires confirmed invoice DOCX/PDF files to be stored only in stable per-customer folders under the approved `rc_finance` Drive root. Production render and delivery staging is permitted only in RAM and must be removed immediately; no final invoice copy may persist on the server. The private SQLite ledger remains authoritative and stores Drive references and document hashes. F19 customer administration and one-way Sheet mirroring remain unchanged. F17A delivery, F17B verified-contact replies, and all confirmation boundaries remain unchanged. Broad messaging and execution tools remain denied.
